...
from Safe_repo.assets.functions import video_metadata
...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [Goku ❤️](https://t.me/src_goku)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**Bot made by https://t.me/src_goku**',
                upm,
                time.time()
            )
        )
...
@Safe_repo.on(events.NewMessage(incoming=True, pattern='/auth'))
...
@Safe_repo.on(events.NewMessage(incoming=True, pattern='/clean'))
...
@Safe_repo.on(events.NewMessage(incoming=True, pattern='/unauth'))
...
@Safe_repo.on(events.NewMessage(incoming=True, pattern='/lock'))
